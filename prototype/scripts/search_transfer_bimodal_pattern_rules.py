...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Search a diagnosis-driven protected-transfer family with one trust gain band plus two abstain bands: "
            "a narrow positive-delta borderline band and a high-pair negative-delta band."
        )
    )
...
def candidate_rules(base_rules: dict) -> Iterable[dict]:
    ...
    reference.setdefault("notes", []).append(
        "Bimodal pattern search keeps the current pattern-safe routing frontier as a reference row."
    )
    yield reference
    ...
    base_notes.append(
        "Bimodal search encodes two distinct abstain modes found in the sample-level diagnosis: a narrow positive-delta borderline band and a high-pair negative-delta band."
    )

    trust_thresholds = [0.445, 0.44]
    trust_min_pair_probs = [0.46, 0.50]
    trust_min_current_probs = [None, 0.44]

    pos_pair_probs = [0.38, 0.40]
    pos_current_probs = [0.46, 0.47]
    pos_conf_gaps = [0.04]
    pos_max_delta_abs_values = [0.09, 0.10]

    neg_pair_probs = [0.60, 0.64]
    neg_bit_accuracies = [0.99, 0.996]
    ...
    for trust_threshold in trust_thresholds:
        for trust_min_pair_prob in trust_min_pair_probs:
            for trust_min_current_prob in trust_min_current_probs:
                for pos_pair_prob in pos_pair_probs:
                    for pos_current_prob in pos_current_probs:
                        for pos_conf_gap in pos_conf_gaps:
                            for pos_max_delta_abs in pos_max_delta_abs_values:
                                for neg_pair_prob in neg_pair_probs:
                                    for neg_min_bit_accuracy in neg_bit_accuracies:
                                        rules = deepcopy(base)
                                        patterns = [
                                            {
                                            ...
                                            {
                                                "name": "abstain_pos_borderline",
                                                "allowed_route_labels": ["abstain"],
                                                "protected_passive_fake_threshold": 0.45,
                                                "protected_delta_abs_anomaly_threshold": 0.35,
                                                "min_unprotected_pair_fake_prob": pos_pair_prob,
                                                "min_protected_passive_prob": pos_current_prob,
                                                "max_protected_passive_prob": 0.495,
                                                "min_bit_accuracy": 0.98,
                                                "max_passive_conf_gap": pos_conf_gap,
                                                "min_passive_delta": 0.03,
                                                "max_passive_delta_abs": pos_max_delta_abs,
                                            },
                                            {
                                                "name": "abstain_highpair_neg",
                                                "allowed_route_labels": ["abstain"],
                                                "protected_passive_fake_threshold": 0.45,
                                                "protected_delta_abs_anomaly_threshold": 0.35,
                                                "min_unprotected_pair_fake_prob": neg_pair_prob,
                                                "min_protected_passive_prob": 0.46,
                                                "max_protected_passive_prob": 0.495,
                                                "min_bit_accuracy": neg_min_bit_accuracy,
                                                "max_passive_conf_gap": 0.04,
                                                "max_passive_delta": 0.0,
                                            },
                                        ]
                                        ...
                                        notes.append(
                                            "This candidate tries to keep the good abstain rescues while filtering abstain false positives, so trust can be made slightly more aggressive without losing the protected-real floor."
                                        )
                                        rules["notes"] = notes
                                        yield rules
...
def pattern_summary(rules: dict) -> dict:
    ...
    for pattern in patterns:
        ...
        elif name in {"abstain_pos_borderline", "abstain_band"}:
            summary["pos_pair_prob"] = f"{float(pattern['min_unprotected_pair_fake_prob']):.6f}"
            if "min_protected_passive_prob" in pattern:
                summary["pos_current_prob"] = f"{float(pattern['min_protected_passive_prob']):.6f}"
            if "max_passive_conf_gap" in pattern:
                summary["pos_max_conf_gap"] = f"{float(pattern['max_passive_conf_gap']):.6f}"
            if "max_passive_delta_abs" in pattern:
                summary["pos_max_delta_abs"] = f"{float(pattern['max_passive_delta_abs']):.6f}"
        elif name == "abstain_highpair_neg":
            summary["neg_pair_prob"] = f"{float(pattern['min_unprotected_pair_fake_prob']):.6f}"
            summary["neg_min_bit_accuracy"] = f"{float(pattern['min_bit_accuracy']):.6f}"
    return summary
