...
def select_sources(root: Path, extensions: Iterable[str], max_videos: int, selector: str) -> List[Path]:
    candidates: List[Path] = []
    for video_dir in discover_video_dirs(root):
        frames = discover_media_files(video_dir, extensions)
        if not frames:
            continue
        candidates.append(select_frame(frames, selector))
...
def build_records(config: dict) -> List[SampleRecord]:
    ...
    max_real_videos = int(dataset_cfg.get("max_real_videos", 16))
    max_fake_videos = int(dataset_cfg.get("max_fake_videos", 16))
    frame_selector = dataset_cfg.get("frame_selector", "middle")
    ...
    real_files = select_sources(real_root, extensions, max_real_videos, frame_selector)
    fake_files = select_sources(fake_root, extensions, max_fake_videos, frame_selector)
        ...
        for variant in derivations_cfg.get("protected_variants", []):
            ...
            records.append(
                SampleRecord(
                    ...
                    pair_group_id=pair_group_id,
                    notes=f"video-dir sampled protected variant {variant_name}",
                )
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a smoke-test manifest by sampling one frame from each video directory.")
