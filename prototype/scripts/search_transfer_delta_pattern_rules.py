...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Search a compact delta-aware protected-transfer pattern family that explicitly uses "
            "pair baseline together with passive-delta shape."
        )
    )
...
def apply_delta_mode(pattern: dict, mode: str | None) -> None:
    if mode is None or mode == "none":
        return
    if mode == "trust_neg":
        pattern["max_passive_delta"] = 0.0
        return
    if mode == "trust_tight_abs":
        pattern["max_passive_delta_abs"] = 0.02
        return
    if mode == "abstain_pos":
        pattern["min_passive_delta"] = 0.05
        pattern["max_passive_delta"] = 0.20
        return
    if mode == "abstain_neg":
        pattern["min_passive_delta"] = -0.20
        pattern["max_passive_delta"] = 0.0
        return
    if mode == "abstain_mid_abs":
        pattern["min_passive_delta_abs"] = 0.05
        pattern["max_passive_delta_abs"] = 0.20
        return
...
def candidate_rules(base_rules: dict) -> Iterable[dict]:
    ...
    reference.setdefault("notes", []).append(
        "Delta-aware search keeps the current pattern-safe routing frontier as a reference row."
    )
    yield reference
    ...
    base_notes.append(
        "Delta-aware search adds explicit pair-delta shape constraints on top of route label and pair baseline."
    )

    trust_thresholds = [0.45, 0.445]
    trust_min_pair_probs = [0.46, 0.50]
    trust_delta_modes = [None, "trust_neg", "trust_tight_abs"]

    abstain_min_pair_probs = [0.20, 0.25, 0.30]
    abstain_min_current_probs = [0.45, 0.46]
    abstain_max_conf_gaps = [0.04, 0.045]
    abstain_delta_modes = [None, "abstain_pos", "abstain_neg", "abstain_mid_abs"]

    for trust_threshold in trust_thresholds:
        for trust_min_pair_prob in trust_min_pair_probs:
            for trust_delta_mode in trust_delta_modes:
                for abstain_min_pair_prob in abstain_min_pair_probs:
                    for abstain_min_current_prob in abstain_min_current_probs:
                        for abstain_max_conf_gap in abstain_max_conf_gaps:
                            for abstain_delta_mode in abstain_delta_modes:
                                rules = deepcopy(base)
                                trust_pattern = {
                                    "name": "trust_delta_band",
                                ...
                                apply_delta_mode(trust_pattern, trust_delta_mode)

                                abstain_pattern = {
                                    "name": "abstain_delta_band",
                                ...
                                apply_delta_mode(abstain_pattern, abstain_delta_mode)

                                rules["protected_transfer_patterns"] = [trust_pattern, abstain_pattern]
                                notes = list(base_notes)
                                notes.append(
                                    "This candidate uses explicit delta-shape constraints to distinguish "
                                    "source-family-supported borderline cases from canonical false-positive lookalikes."
                                )
                                rules["notes"] = notes
                                yield rules
...
def pattern_summary(rules: dict) -> dict:
    ...
    if not patterns:
        return summary
    summary["mode"] = "delta_pattern"
    for pattern in patterns:
        name = pattern.get("name")
        if name in {"trust_delta_band", "trust_band"}:
            if name == "trust_band":
            ...
            if "max_passive_delta" in pattern and float(pattern["max_passive_delta"]) == 0.0:
                summary["trust_delta_mode"] = "trust_neg"
            elif "max_passive_delta_abs" in pattern and float(pattern["max_passive_delta_abs"]) <= 0.02:
                summary["trust_delta_mode"] = "trust_tight_abs"
        elif name in {"abstain_delta_band", "abstain_band"}:
            ...
            if "min_passive_delta" in pattern and "max_passive_delta" in pattern:
                min_delta = float(pattern["min_passive_delta"])
                max_delta = float(pattern["max_passive_delta"])
                if min_delta >= 0.05 and max_delta <= 0.20:
                    summary["abstain_delta_mode"] = "abstain_pos"
                elif min_delta <= -0.20 and max_delta == 0.0:
                    summary["abstain_delta_mode"] = "abstain_neg"
            elif "min_passive_delta_abs" in pattern and "max_passive_delta_abs" in pattern:
                summary["abstain_delta_mode"] = "abstain_mid_abs"
    return summary
