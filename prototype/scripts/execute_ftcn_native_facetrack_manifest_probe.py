...
# FTCN's released inference code predates NumPy 1.24.
...
def load_native_video_state(video_path: Path, ftcn_root: Path, max_frame: int):
    install_ftcn_path(ftcn_root)
    from test_tools.common import detect_all, grab_all_frames
    from test_tools.ct.operations import find_longest, multiple_tracking
    from test_tools.utils import get_crop_box

    cache_dir = project_root() / "workspace" / "cache" / "ftcn_native_facetrack"
    ...
    *,
    ...
    for clip in windows:
        try:
            ...
            preds.append(float(output["final_output"].reshape(-1)[0].detach().cpu()))
        except Exception:
            continue
    if not preds:
...
def parse_args() -> argparse.Namespace:
    root = project_root()
    parser = argparse.ArgumentParser(description="Run official FTCN+TT native face-track video inference on manifest rows.")
    ...
    parser.add_argument("--checkpoint", default=str(root / "workspace" / "repos" / "FTCN" / "checkpoints" / "ftcn_tt.pth"))
...
def main() -> None:
    ...
    classifier, clip_size, imsize = load_ftcn(ftcn_root, Path(args.checkpoint).resolve(), args.device)
    install_ftcn_path(ftcn_root)
    from test_tools.faster_crop_align_xray import FasterCropAlignXRay
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        for index, row in enumerate(tqdm(rows, desc="ftcn-native-rows"), start=1):
            ...
            if video_key not in state_cache:
                if args.max_videos > 0 and len(state_cache) >= args.max_videos:
                    ...
                    continue
                state_cache[video_key] = load_native_video_state(video_path, ftcn_root, args.max_frame)
    ...
    print(f"Wrote FTCN native face-track results to {output_path}")
