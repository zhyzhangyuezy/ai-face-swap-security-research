...
from search_transfer_pattern_rules import candidate_rules, sort_key


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Leave-one-dataset-out validation for the pattern-based transfer-aware routing family, "
...
def transfer_thresholds(rules: dict) -> tuple[float | None, float | None]:
    pattern_thresholds = []
    pattern_anomalies = []
    for pattern in rules.get("protected_transfer_patterns") or []:
        if "protected_passive_fake_threshold" in pattern:
            pattern_thresholds.append(float(pattern["protected_passive_fake_threshold"]))
        if "protected_delta_abs_anomaly_threshold" in pattern:
            pattern_anomalies.append(float(pattern["protected_delta_abs_anomaly_threshold"]))

    if pattern_thresholds or pattern_anomalies:
        threshold = min(pattern_thresholds) if pattern_thresholds else float(rules["fallback"]["protected_passive_fake_threshold"])
        anomaly = min(pattern_anomalies) if pattern_anomalies else float(rules["fallback"]["protected_delta_abs_anomaly_threshold"])
...
def main() -> None:
    ...
    for holdout in canonical_paths:
        ...
        threshold, anomaly = transfer_thresholds(best_rules)
        output_row = {
            "holdout_dataset": holdout_name,
            "selected_config_id": str(best_row["config_id"]),
            "selected_pattern_family": "1" if best_rules.get("protected_transfer_patterns") else "0",
            "selected_transfer_threshold": "" if threshold is None else f"{threshold:.6f}",
            "selected_transfer_anomaly_threshold": "" if anomaly is None else f"{anomaly:.6f}",
